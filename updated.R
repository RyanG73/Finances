...
today <- as.Date('2024-12-31')#lubridate::today()
...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    dateRangeInput("dateRange", "Select Date Range:", 
                   start = first_day_this_year, end = last_day_this_month)
  ),
  dashboardBody(
    tabItems(
      tabItem(tabName = "dashboard",
              fluidRow(
                # Summary Overview
                box(title = "Summary Overview", status = "primary", solidHeader = TRUE, width = 12,
                    valueBoxOutput("totalIncome", width = 4),
                    valueBoxOutput("totalSpending", width = 4),
                    valueBoxOutput("totalBalance", width = 4))
              ),
              fluidRow(
              ...
              fluidRow(
                ...
                box(title = "Year-over-Year Comparison", status = "info", solidHeader = TRUE, width = 12,
                    plotOutput("yearComparisonPlot", height = "300px"))
              )
      ),
...
server <- function(input, output) {
  ...
  # Total Income, Spending, Balance Summary
  output$totalIncome <- renderValueBox({
    total_income <- raw %>%
      filter(name == input$name, in_out == "In", date >= input$dateRange[1], date <= input$dateRange[2]) %>%
      summarize(total = sum(amount)) %>%
      pull(total)
    valueBox(format(total_income, big.mark = ","), "Total Income", color = "green")
  })
  
  output$totalSpending <- renderValueBox({
    total_spending <- raw %>%
      filter(name == input$name, in_out == "Out", date >= input$dateRange[1], date <= input$dateRange[2]) %>%
      summarize(total = sum(amount)) %>%
      pull(total)
    valueBox(format(total_spending, big.mark = ","), "Total Spending", color = "red")
  })
  
  output$totalBalance <- renderValueBox({
    total_income <- raw %>%
      filter(name == input$name, in_out == "In", date >= input$dateRange[1], date <= input$dateRange[2]) %>%
      summarize(total = sum(amount)) %>%
      pull(total)
    ...
    valueBox(format(balance, big.mark = ","), "Total Balance", color = ifelse(balance >= 0, "green", "red"))
  })
      ...
      output[[paste0("balanceMonth", month_index)]] <- renderValueBox({
        ...
        if (nrow(month_data) >= month_index) {
          ...
          valueBox(format(balance, big.mark = ","),
                   "Balance", color = ifelse(balance >= 0, "green", "red"))
        } else {
  ...
  output$categoryMonth1Box <- renderValueBox({
    ...
    if (nrow(month_data) >= 1) {
      valueBox(format(month_data$total_spending[1], big.mark = ","), 
               "Category Spending", color = "blue")
    } else {
      valueBox("N/A", "Category Spending", color = "blue")
    }
  })
  ...
  output$yearComparisonPlot <- renderPlot({
    ...
    ggplot(comparison_data, aes(x = month, y = total_spending, color = factor(year))) +
      geom_line(color = "blue", linewidth=2) +
      labs(title = paste("Year-over-Year Spending Comparison:", input$category, "for", input$name),
           x = "Month", y = "Amount", color = "Year") +
      theme_minimal()
  })
}
